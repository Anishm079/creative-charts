...
const barData = [
  {
    name: "SEA",
    range: [new Date(2013, 1, 1), new Date(2019, 1, 1)],
  },
  {
    name: "HKG",
    range: [new Date(2015, 1, 1), new Date(2015, 5, 1)],
  },
  {
    name: "LHR",
    range: [new Date(2016, 5, 1), new Date(2019, 1, 1)],
  },
  {
    name: "DEN",
    range: [new Date(2018, 8, 1), new Date(2019, 1, 1)],
  },
];
...
class DraggablePoint extends React.Component<any, any> {
...
function LinkedBrushing() {
  const [zoomDomain, setZoomDomain] = React.useState({});
  const [bars, setBars] = React.useState(barData);
  const [points, setPoints] = React.useState(pointData);

  function handleZoom(domain: any) {
    setZoomDomain(domain);
  }

  function onDomainChange(domain: any, props: any) {
    const { name } = props;
    const newBars = bars.map((bar) => (bar.name === name ? { name, range: domain } : bar));
    setBars(newBars);
  }

  function onPointChange(point: any) {
    const newPoints = points.map((p) => (p.name === point.name ? point : p));
    setPoints(newPoints);
  }

  return (
    <>
        <h2 style={{color:"black",position:"absolute",width:"100%"}} >event tracking</h2>
        ...
        <VictoryChart
            ...
            <VictoryZoomContainer
                ...
                clipContainerComponent={<VictoryClipContainer clipPadding={{ top: 15, bottom: 15 }} />}
            />
            }
        >
            ...
            <VictoryScatter
            data={points}
            dataComponent={<DraggablePoint onPointChange={onPointChange} />}
            style={{
                data: {
                fill: VictoryTheme.clean.palette?.colors?.cyan,
                opacity: ({ active }) => (active ? 1 : 0.5),
        ...
        >
            ...
            tickCount={3}
            tickFormat={(t: Date) => t.getFullYear()}
            />
            <VictoryScatter
            data={points}
            size={5}
            style={{ data: { fill: VictoryTheme.clean.palette?.colors?.cyan } }}
            x="name"
            y="date"
            />
            <VictoryBar
