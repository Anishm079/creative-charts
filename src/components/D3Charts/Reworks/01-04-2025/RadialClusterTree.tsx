...
const ChordChart: React.FC = () => {
  ...
  useEffect(() => {
    const width = 1080;
    const height = width;
    const innerRadius = Math.min(width, height) * 0.5 - 370;
    ...
    const ribbon = d3.ribbonArrow<any, d3.ChordSubgroup>()
    ...
    const svg = d3.select(svgRef.current)
      .attr("width", width)
      .attr("height", height)
      .attr("viewBox", [-width / 2, -height / 4, width, height].join(" "))
