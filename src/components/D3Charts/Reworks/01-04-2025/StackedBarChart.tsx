...
const ChartComponent: React.FC = () => {
  useEffect(() => {
    ...
    const series = d3.stack<{ age: string, state: string, population: number }>()
    ...
    // Adding thick lines with different color codes from low to high values
    series.forEach(s => {
      const minValue = d3.min(s, d => d[0]);
      const maxValue = d3.max(s, d => d[1]);
      svg.append("line")
        .attr("x1", x(minValue))
        .attr("x2", x(maxValue))
        .attr("y1", marginTop)
  ...
  return (
    <div id="myChart" style={{background:"linear-gradient(90deg, #2f00ff, #0072ff)",width:"100dvw",height:"100dvh"}} />
  );
};
