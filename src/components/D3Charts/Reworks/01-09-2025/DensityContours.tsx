  ...
  useEffect(() => {
    const createChart = () => {
      ...
      const x = d3.scaleLinear()
        .domain(d3.extent(data, d => d.waiting) as [number, number]).nice()
        .rangeRound([marginLeft, width - marginRight]);

      const y = d3.scaleLinear()
        .domain(d3.extent(data, d => d.eruptions) as [number, number]).nice()
        .rangeRound([height - marginBottom, marginTop]);

      // Compute the density contours.
      const contours = d3.contourDensity<DataPoint>()
        .x(d => x(d.waiting))
        .y(d => y(d.eruptions))
        .size([width, height])
        .bandwidth(30)
